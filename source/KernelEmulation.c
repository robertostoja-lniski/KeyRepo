...
int addKeyNodeToPartition(KeyNode* keyNodeToAdd, uint64_t __user *id) {
    ...
    if(set_buffered_file(partition, (char** )&mappedPartition, fileSizeAdd + keyNodeToAdd->keySize) != fileSizeAdd) {
...
int addKeyNodeByPartitionPointer(void* mappedPartition, KeyNode* keyNodeToAdd, uint64_t __user *id) {
    ...
    for(i = 0; i < mapSize; i++) {
        ...
        if (currentId == 0) {
            ...
            if (keyNodeToAdd->keySize < KEY_PARTITION_NODE_INITIAL_SIZE) {
