...
int do_write_key(const char* key, const uint64_t keyLen, uint64_t* id, int uid, int gid, uint8_t type);
int do_read_key(const uint64_t id, char* key, uint64_t keyLen, int uid, int gid, uint8_t type);
